...
func (g *Gold) GetPrices() (*Price, error) {
	...
	if g.Client == nil {
		log.Println("setting client to default client")
	...
	gold := Gold{}
	var previos, current, change float64

	err = json.Unmarshal(body, &gold)
	if err != nil {
		log.Println("Error unmashalling", err)

		return nil, err
	}

	previos, current, change = gold.Prices[0].PreviousClose, gold.Prices[0].Price, gold.Prices[0].Change

	var currentInfo = Price{
		Currency:      currency,
		Price:         current,
		Change:        change,
		PreviousClose: previos,
		Time:          time.Now(),
	}
